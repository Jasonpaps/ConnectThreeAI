...
class Connect:
    def __init__(
        self, starting_player="x", num_cols=5, num_rows=3, num_connect=3, verbose=True
    ):
        """
        ...
        self._other_player = {
            "o": "x",
            "x": "o",
        }  # This is for changing between players, e.g.,  self._other_player['o'] yields 'x'.

        # Define two-dimensional array of strings to represent the grid.
        self.grid = np.full(
            fill_value=" ", shape=(self._num_rows, self._num_cols), dtype=str
        )
        ...
        if self._starting_player not in self._players:
            raise ValueError(
                "The argument starting_player has to be either 'x', or 'o'."
            )
        else:
    ...
    def act(self, action):
        ...
        self._lowest_free_row_per_column[
            action
        ] += 1  # You can ignore this; internal use only.
